...
**5. SCORE** — LLM call, but narrow. For one candidate against one rubric criterion at a time, the model returns a verdict (pass/fail or a rating), a confidence, and a quoted piece of evidence from the CV. It does not produce a final score — that would mean re-running the whole judgement to check its own maths, and it would drift between runs. One criterion in, one verdict out, repeated per criterion.

**6. AGGREGATE** — Pure code. Takes all the per-criterion verdicts from SCORE and does the arithmetic: binary gates for must-haves (any failed must-have caps the candidate, however good the rest of the CV is), weighted sum for nice-to-haves. Weights live in editable rubric config so an agency can tune them without a code change.

**7. DEDUPE** — Catches the same person applying to multiple roles under slightly different details (a shortened name, a different email — see the duplicate-planting logic in `generate_cvs.py`). Cascades from cheap to expensive: exact email match first, then fuzzy name+phone matching, then embedding similarity against application history only if the cheaper checks don't resolve it. Embeddings are the one place semantic similarity earns its cost here.
